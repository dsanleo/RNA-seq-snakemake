...
file=snakemake@input[["countsFile"]]
...
table=read.table(file,header=TRUE,sep="\t",comment.char = "_")
row.names(table)=sub("\\.[0-9]+","",table[,1])
countTable=table[,2:ncol(table)]
...
finalTable=merge(finalTable,counts(dds,normalized=FALSE),by.x="row.names",by.y="row.names")
row.names(finalTable)=finalTable$Row.names
finalTable$Row.names=c()
finalTableNorm=merge(finalTable,counts(dds,normalized=TRUE),by.x="row.names",by.y="row.names",suffixes=c("",".norm"))
#finalTableNorm=finalTableNorm[,c()]
write.table(finalTable,file=diffExpTable,sep="\t",quote=FALSE)
write.table(finalTable,file=paste0(diffExpTable,".norm.csv"),sep="\t",quote=FALSE)
